...
impl<T> Grid<T> {
    ...
    fn index_to_coord(&self, index: usize) -> Vector {
        ...
        return Vector{ x: x.try_into().unwrap(), y: y.try_into().unwrap() };
    }
}
...
fn part_1(input: &ParsedInput) -> String {
    ...
    let grid = &input.grid;
    let sum = grid.elements.iter().enumerate().fold(0, |sum, (index, grid_char)| {
        if grid_char != pattern_chars.get(0).unwrap() { return sum }
        let starting_coord = grid.index_to_coord(index);
        let res = Vector::get_directions().iter().fold(0, |inner_sum, direction| {
...
fn parse_input(input: &str) -> ParsedInput {
    let lines = input.lines().map(|line_str| {
    ...
    let chars = lines.iter().flatten().map(|char| {
        char.clone()
    }).collect::<Vec<char>>();
    ParsedInput {
        grid: Grid {
...
fn check_pattern(grid: &Grid<char>, pattern: &Vec<char>, starting_coord: &Vector, direction: &Vector) -> bool {
    let mut current_coord: Vector = Vector{x: starting_coord.x, y: starting_coord.y};
    ...
    println!("Pattern found at: {:?}, Direction: {:?}", starting_coord, direction);
    return true;
}
